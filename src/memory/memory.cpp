...
bool c_memory::DumpMemoryMap(bool debug)
{
...
bool c_memory::Init(std::string process_name, bool memMap, bool debug)
{
	if (!DMA_INITIALIZED)
	{
		LOG("inizializing...\n");
...
DWORD c_memory::GetPidFromName(std::string process_name)
{
	DWORD pid = 0;
...
std::vector<int> c_memory::GetPidListFromName(std::string name)
{
...
std::vector<std::string> c_memory::GetModuleList(std::string process_name)
{
...
VMMDLL_PROCESS_INFORMATION c_memory::GetProcessInformation()
{
...
size_t c_memory::GetBaseAddress(std::string module_name)
{
...
size_t c_memory::GetBaseSize(std::string module_name)
{
	std::wstring str(module_name.begin(), module_name.end());
...
uintptr_t c_memory::GetExportTableAddress(std::string import, std::string process, std::string module)
{
...
uintptr_t c_memory::GetImportTableAddress(std::string import, std::string process, std::string module)
{
...
bool c_memory::DumpMemory(uintptr_t address, std::string path)
{
...
uint64_t c_memory::FindSignature(const char* signature, uint64_t range_start, uint64_t range_end, int PID)
{
...
bool c_memory::Write(uintptr_t address, void* buffer, size_t size, int pid) const
{
	if (!(address > 0x2000000 && address < 0x7FFFFFFFFFFF))
		return false;
...
bool c_memory::Read(uintptr_t address, void* buffer, size_t size, int pid) const
{
...
VMMDLL_SCATTER_HANDLE c_memory::CreateScatterHandle()
{
...
VMMDLL_SCATTER_HANDLE c_memory::CreateScatterHandle(int pid)
{
